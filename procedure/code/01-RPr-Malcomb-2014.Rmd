...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Grayson Shanley Barr, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
# add line here to save new file  for second vulnerability score
```


### Recalculating Vulnerability Score: A multiplicative approach

Calculate an aggregated vulnerability score by multiplying low adaptive capacity (invert adaptive capacity by subtracting from the maximum score of 40), livelihood sensitivity, and drought exposure plus flood risk. Calculating a multiplicative vulnerability score 

$$
Multiplicative Vulnerability Score = (40 - Adaptive Capacity) * (Livelihood Sensitivity) * (Drought Exposure + Flood Risk)
$$

```{r combined-vulnerability-multiplicative}
# final output (multiplying component rasters)
vulnerability_2 <- (40 - capacity_r) * (sensitivity_r) * (drought_r + flood_r) 
# 100 = highest vulnerability, 0 = lowest vulnerability
names(vulnerability_2) <- "vulnerability"
plot(vulnerability_2)
```

```{r find break & limit values for vulnerability map number 2}
vuln_max <- max(vulnerability_2[[1]],na.rm = TRUE)
vuln_min <- min(vulnerability_2[[1]],na.rm = TRUE)
```

### Reproduction figure 6 - visualizing a multiplcative vulnerability score

```{r vulnerability-map-2}
vuln_map2 = ggplot() +
  geom_stars(data = vulnerability_2) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(vuln_min, vuln_max),
  ...
  labs(title = "Malawi Vulnerability to Climate Change (Multiplicative)") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
# Discussion
In a reproduction analysis of Malcomb (2014), an multiplicative score was calculated to compare results with the original use of an additive vulnerability aggregation method in the original model. 

Vulnerability scores from the additive model have a range of 33.08 - 82.94. Meanwhile, vulnerability scores from the multiplicative model have a range of 0 - 16,209.26. Visually inspecting the map outputs from both scores demonstrate that when using the additive vulnerability score there are more regions documented with moderate to high vulnerability (Figure 5). Meanwhile, using a multiplicative vulnerability score there are more areas with low vulnerability, with the exceptions of Northern and Southern Malawi which exhibit high vulnerability scores in both maps (Figures 5 and 6). 

# Conclusions
There are considerable differences in results between using additive or multiplicative aggregation methods to calculate vulnerability scores. Using an additive aggregation method, there are more regions that document higher vulnerability. In addition, no regions are able to have a vulnerability score of 0. Meanwhile, implementing a multiplicative aggregation method to calculate vulnerability allows for a substantially greater range of vulnerability scores including a value of 0.

The implications of these differences in ranges are clearly observed when comparing maps in Figures 5 and 6. For instance, the additive method identified many more regions of Malawi as experiencing high vulnerability, while the multiplicative score predominantly highlighted the most vulnerable regions. This is because when implementing a multiplicative aggregation method there is a larger range and  substantially more values were captured within the intermediate range of vulnerability. 

The results of the reproduction analysis have implications for the prioritization of development projects. For example, by implementing  and comparing additive and multiplicative aggregation methods can assist in identifying the most vulnerable regions of Malawi to climate change where adaptation projects should be prioritized. Furthermore, development projects should first prioritize regions with high vulnerability documented by the multiplicative vulnerability score and subsequently prioritize highly vulnerable regions identified by the additive score. 

In sum, in concurrence with previous studies examining adaptive capacity and vulnerability (Liu & Kontu, 2022) generating a composite vulnerability score as an additive index is effective when estimating the impact of adaptive capacity interventions, however the multiplicative index is more effective with physical exposure and livelihood sensitivity metrics are included in vulnerability models. 
